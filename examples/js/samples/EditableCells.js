myCompanyNameSpace.SAMPLE_CONFIGS_DETAIL_URLS["EditableCells"]="http://www.flexicious.com/resources/Ultimate/docs/EditableCells.htm";

/**
 * Flexicious
 * Copyright 2011, Flexicious LLC
 */
(function(window)
{
    "use strict";
    var ComboBoxItemEditor, uiUtil = flexiciousNmsp.UIUtils, flxConstants = flexiciousNmsp.Constants;
    /**
     * A ComboBoxItemEditor is a custom item renderer, that defines how to use custom cells with logic that you can control
     * @constructor
     * @namespace com.flexicious.controls
     * @extends UIComponent
     */
    ComboBoxItemEditor=function(){
        //make sure to call constructor
        flexiciousNmsp.ComboBox.apply(this);//we extend from the built in flexicious combobox, which gives us a bunch of properties (like selected value etc)
        /**
         * This is a getter/setter for the data property. When the cell is created, it belongs to a row
         * The data property points to the item in the grids dataprovider that is being rendered by this cell.
         * @type {*}
         */
        this.data=null;

        //the add event listener will basically proxy all DomEvents to your code to handle.
        this.addEventListener(this,flxConstants.EVENT_CHANGE,this.onChange);
    };
    myCompanyNameSpace.EditableCells_ComboBoxItemEditor = ComboBoxItemEditor; //add to name space
    ComboBoxItemEditor.prototype = new flexiciousNmsp.ComboBox(); //setup hierarchy
    ComboBoxItemEditor.prototype.typeName = ComboBoxItemEditor.typeName = 'ComboBoxItemEditor';//for quick inspection
    ComboBoxItemEditor.prototype.getClassNames=function(){
        return ["ComboBoxItemEditor","UIComponent"]; //this is a mechanism to replicate the "is" and "as" keywords of most other OO programming languages
    };

    /**
     * This is important, because the grid looks for a "setData" method on the renderer.
     * In here, we intercept the call to setData, and inject our logic to populate the combo box.
     * @param val
     */
    ComboBoxItemEditor.prototype.initialize=function(){
        flexiciousNmsp.UIComponent.prototype.initialize.apply(this);
        this.dataField="code";// the code field is used on the data provider to match the item to select.
        this.labelField="name";//to display the text of the dropdown
        this.setDataProvider(flexiciousNmsp.SystemConstants.states);//list of all states we want as the dataprovider;
        var cell = this.grid.getCurrentEditCell();//each editor gets a grid parameter. Also parent object of this cell is either the leftLocked or the rightLocked, or the unlocked containers, all of which have a grid property.
        this.setValue(cell.getRowInfo().getData().headquarterAddress.state.code);
    };
    /**
     * This event is dispatched when the user clicks on the icon. The event is actually a flexicious event, and has a trigger event
     * property that points back to the original domEvent.
     * @param event
     */
    ComboBoxItemEditor.prototype.onChange=function(evt){

        //in the editor you have a handle to the grid iteself.
        this.grid.getCurrentEditCell().getRowInfo().getData().headquarterAddress.state=this.getSelectedItem();

    }
}(window));


myCompanyNameSpace.editableCells_getRowDisabled=function (cell, data){

    if(data.legalName=='Adobe Systems'){
        return true;
    }
    return false;//do not disable by default.

};
myCompanyNameSpace.editableCells_flexdatagridcolumn1_itemEditValueCommitHandler=function (evt){

    if(evt.column.dataField=="headquarterAddress.city.name"){
        var txt=evt.itemEditor["text"];
        var found=false;
        var items=flexiciousNmsp.SystemConstants.cities;
        for(var i=0;i<items.length;i++){
            var city=items[i];
            if(city.name==txt){
                evt.item.headquarterAddress.city=city;
                found=true;
                break;
            }
        }
        if(!found){
            window.alert("Invalid City Entered: " + txt);
        }
        event.preventDefault();//we do this, so when the value is entered, we validate and apply ourselves, dont let the grid apply it..
    }

};


myCompanyNameSpace.editableCells_validateFutureDate=function (editor){
     var cell = grid.getCurrentEditCell();
    if((editor.getSelectedDate()<new Date() ) &&
        (editor.getSelectedDate().toString()!= cell.getRowInfo().getData().addedDate.toString())){
        window.alert("Please choose a date in the future.");
        return false;
    }
    return true;

};
myCompanyNameSpace.SAMPLE_CONFIGS["EditableCells"]='<grid enableDrag="true" rowDisabledFunction="myCompanyNameSpace.editableCells_getRowDisabled" editable="true" enableFilters="true" '+
    '										 enableFooters="true" id="grid"'+
    '										 itemEditValueCommit="myCompanyNameSpace.editableCells_flexdatagridcolumn1_itemEditValueCommitHandler" preferencePersistenceKey="editableCells">'+
    '			<columns>'+
    '				<column type="checkbox" />'+
    '				<column dataField="id" headerText="ID" filterControl="TextInput" '+
    '													   editable="true" selectable="true" truncateToFit="true"/>'+
    '				<column dataField="legalName" headerText="Legal Name" truncateToFit="true" selectable="true"'+
    '													   editable="true" />'+
    '				<column dataField="headquarterAddress.line1" headerText="Address Line 1" footerLabel="Count:" '+
    '													   footerOperation="count" editable="true"/>'+
    '				<column dataField="headquarterAddress.line2" headerText="Address Line 2"/>'+
    '				<column dataField="headquarterAddress.city.name" headerText="City" filterControl="MultiSelectComboBox" filterComboBoxBuildFromGrid="true" filterComboBoxWidth="150" editable="true" />'+
    '				<column dataField="headquarterAddress.state.name" headerText="State" '+
    '													   filterControl="MultiSelectComboBox" filterComboBoxBuildFromGrid="true" '+
    '													   filterComboBoxWidth="150" editable="true" '+
    '    itemEditorManagesPersistence="true" useFilterDataProviderForItemEditor="true" itemEditor="myCompanyNameSpace.EditableCells_ComboBoxItemEditor">'+
    '				</column>'+
    '				<column dataField="headquarterAddress.country.name" headerText="Country" filterControl="MultiSelectComboBox" filterComboBoxBuildFromGrid="true" ' +
    '   filterComboBoxWidth="150" editable="false"/>'+
    '				<column dataField="addedDate" headerText="Added Date" '+
    '													   itemEditor="flexiciousNmsp.DatePicker" format="date"  '+
    '													   editable="true" editorDataField="selectedDate" />'+
    '				<column dataField="annualRevenue" headerText="Annual Revenue" textAlign="right" headerAlign="center" '+
    '													   footerLabel="Avg:" footerOperation="average" footerAlign="center" footerOperationPrecision="2"'+
    '													   footerFormatter="flexiciousNmsp.CurrencyFormatter" '+
    '													   labelFunction="flexiciousNmsp.UIUtils.dataGridFormatCurrencyLabelFunction" editable="true"/>'+
    '				<column dataField="numEmployees" headerText="Num Employees" textAlign="right" footerLabel="Avg:" '+
    '													   footerOperation="average" footerOperationPrecision="2" footerFormatter="flexiciousNmsp.CurrencyFormatter"'+
    '													   labelFunction="flexiciousNmsp.UIUtils.dataGridFormatCurrencyLabelFunction" editable="true"/>'+
    '				<column dataField="earningsPerShare" headerText="EPS" textAlign="right" '+
    '													   footerLabel="Avg:" footerOperation="average" footerFormatter="flexiciousNmsp.CurrencyFormatter" '+
    '													   labelFunction="flexiciousNmsp.UIUtils.dataGridFormatCurrencyLabelFunction" editable="true"/>'+
    '				<column dataField="lastStockPrice" headerText="Stock Price" '+
    '													   textAlign="right" footerLabel="Avg:" footerOperation="average" '+
    '													   footerOperationPrecision="2" footerFormatter="flexiciousNmsp.CurrencyFormatter" '+
    '													   labelFunction="flexiciousNmsp.UIUtils.dataGridFormatCurrencyLabelFunction" editable="true"/>'+
    '			</columns>'+
    '		</grid>';